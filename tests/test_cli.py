from configmap_reader.cli import run
...
def test_run_show_version(monkeypatch, capsys, option_version):
    monkeypatch.setattr(
        "sys.argv",
        ["configmap-reader", option_version],
    )
...
def test_run_wrong_options(monkeypatch, capsys, options):
    ...
    captured = capsys.readouterr()
    assert "usage: configmap-reader [-h] [-v]" in captured.err
    assert "configmap-reader: error: unrecognized arguments:" in captured.err
